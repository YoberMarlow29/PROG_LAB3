...
class Usuario
{
    ...
    public function VerificarUsuario(Request $request, Response $response, array $args): Response
    {
        ...
        if (isset($arrayDeParametros["user"])) {
            ...
            if ($usuario = Usuario::TraerUsuarioClaveContraseña($obj)) {
                $data = array();
                unset($usuario->clave);
                $alumno = array("nombre"=>"Yober Marlow", "apellido"=>"Vasquez Panduro");
                $dni_alumno = array("dni_alumno"=>"44600384");
    ...
    public static function TraerUsuarioClaveContraseña($obj)
    {
        $accesoDatos = AccesoDatos::obtenerObjetoAccesoDatos();
        $consulta = $accesoDatos->retornarConsulta(
            "SELECT * FROM usuarios
